...
public class MBTA {
  ...
  public synchronized boolean isStationOccupied(Station s, Train excludeTrain) {
    for (Map.Entry<Train, Station> entry : trainLocations.entrySet()) {
      if (!entry.getKey().equals(excludeTrain) && entry.getValue().equals(s)) {
        return true;
      }
    }
    return false;
  ...
  public synchronized void moveTrain(Train t, Station from, Station to) {
    ...
    if (toIndex > fromIndex) {
      trainDirections.put(t, 1);
    } else {
      trainDirections.put(t, -1);
    }
    
    if (toIndex == line.size() - 1) {
      trainDirections.put(t, -1);
    } else if (toIndex == 0) {
      trainDirections.put(t, 1);
    }
  ...
  public synchronized Station getNextStation(Train t) {
    List<Station> line = getTrainLine(t);
  ...
  public synchronized void boardPassenger(Passenger p, Train t) {
    passengerTrains.put(p, t);
  }
  
  public synchronized void deboardPassenger(Passenger p, Station s) {
  ...
  public synchronized boolean isPassengerDone(Passenger p) {
    List<Station> journey = getPassengerJourney(p);
    int progress = passengerProgress.get(p);
    return progress >= journey.size() - 1;
  }

  public synchronized Station getPassengerLocation(Passenger p) {
    return passengerLocations.get(p);
  }
  
  public synchronized Train getPassengerTrain(Passenger p) {
    return passengerTrains.get(p);
  }
  
  public synchronized Station getTrainLocation(Train t) {
    return trainLocations.get(t);
  }
  
  public synchronized Station getPassengerNextStation(Passenger p) {
    List<Station> journey = getPassengerJourney(p);
    int progress = passengerProgress.get(p);
  ...
  public synchronized int getPassengerProgress(Passenger p) {
    return passengerProgress.get(p);
  }
  
  public synchronized int getTrainPosition(Train t) {
    return trainPositions.get(t);
  }
  
  public synchronized int getTrainDirection(Train t) {
